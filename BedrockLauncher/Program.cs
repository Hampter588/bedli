...
namespace BedrockLauncher
{
    public static class Program
    {
        ...
        internal static async Task EnsureStoreEngagementForCliAsync(Version minimumVersion, string architecture)
        {
            if (IsStoreEngagementInstalled(minimumVersion, architecture)) return;
            if (!string.Equals(architecture, "x64", StringComparison.OrdinalIgnoreCase))
                throw new InvalidOperationException($"Minecraft requires Microsoft.Services.Store.Engagement {minimumVersion} for {architecture}. Automatic installation currently supports x64 only.");

            Console.WriteLine("Installing required Microsoft Store Engagement framework...");
            string packagePath = await DownloadStoreEngagementPackage();
            await InstallStoreEngagementPackage(packagePath);
            if (!IsStoreEngagementInstalled(minimumVersion, architecture))
                throw new InvalidOperationException($"Microsoft.Services.Store.Engagement {minimumVersion} or newer ({architecture}) is still missing after installation.");
            Console.WriteLine("Microsoft Store Engagement framework is ready.");
        }

        private static bool IsStoreEngagementInstalled(Version requiredVersion = null, string architecture = "x64")
        {
            Version minimumVersion = requiredVersion ?? new Version(StoreEngagementMinimumVersion);
            ...
            try
            {
                ...
                {
                    string packageArchitecture = package.Id.Architecture.ToString();
                    if (!string.Equals(packageArchitecture, architecture, StringComparison.OrdinalIgnoreCase) &&
                        !string.Equals(packageArchitecture, "neutral", StringComparison.OrdinalIgnoreCase)) continue;
