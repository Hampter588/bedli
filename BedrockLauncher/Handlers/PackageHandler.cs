...
namespace BedrockLauncher.Handlers
{
    public class PackageHandler : IDisposable
    {
        ...
        // The bundled launcher_core.dll requires this process filename.
        // Keep the public CLI named bedli; only the extraction child uses this name.
        private const string MsixvcExtractorHelperFileName = "LeviLauncher.exe";
            ...
            try
            {
                ...
                if (!LaunchEditor && v.PackageType == PackageType.GDK && v.IsInstalledInLauncher)
                {
                    if (await TryLaunchLocalGdkExecutable(v, KeepLauncherOpen))
                    {
                        ...
                        if (!KeepLauncherOpen && !Program.IsCommandLine)
        ...
        public async Task InstallPackage(MCVersion v, string dirPath)
        {
            try
            {
                ...
                if (Program.IsCommandLine)
                    EnsureLocalRegistrationIsAvailable(v);
                ...
                if (Program.IsCommandLine)
                {
                    XDocument manifest = XDocument.Load(v.ManifestPath);
                    XElement engagement = manifest.Descendants().FirstOrDefault(element =>
                        element.Name.LocalName == "PackageDependency" &&
                        (string)element.Attribute("Name") == "Microsoft.Services.Store.Engagement");
                    if (engagement != null)
                        await Program.EnsureStoreEngagementForCliAsync(
                            Version.Parse((string)engagement.Attribute("MinVersion") ?? "0.0.0.0"), v.Architecture);
                }
        ...
        private string EnsureMsixvcExtractorHelperExecutable()
        {
            ...
            if (string.IsNullOrWhiteSpace(currentExecutable) || !File.Exists(currentExecutable))
                currentExecutable = Path.Combine(AppContext.BaseDirectory, "bedli.exe");
        ...
        private string BuildGdkExtractionFailureMessage(MCVersion v, string nativeExtractionProblem, string helperExtractionProblem)
        {
            ...
            return
                ...
                "Your Microsoft Store Minecraft install was not changed. " +
                "The download is cached; retrying will reuse it.";
        }
        ...
        {
            foreach (string directory in GetRegisteredVCLibsRuntimeDirectories())
        ...
        private void EnsureLocalRegistrationIsAvailable(MCVersion version)
        {
            foreach (Package package in PM.FindPackagesForUser(string.Empty, Constants.GetPackageFamily(version.Type)))
            {
                string location = GetPackageInstalledLocation(package);
                if (IsPathInside(MainDataModel.Default.FilePaths.VersionsFolder, location))
                    continue;

                var installed = package.Id.Version;
                throw new InvalidOperationException(
                    $"Cannot register Minecraft {version.Name}: Windows already has {package.Id.Name} " +
                    $"{installed.Major}.{installed.Minor}.{installed.Build}.{installed.Revision} installed outside bedli. " +
                    "Windows cannot register this local UWP version alongside that installation with the same package identity. " +
                    "Back up your worlds before replacing the existing installation. " +
                    "Bedli has not removed it; existing downloads and extracted files are kept.");
            }
        }

        private async Task LaunchExactVersion(MCVersion version)
        {
            string gamePath = Path.GetFullPath(Path.Combine(version.GameDirectory, "Minecraft.Windows.exe"));
            if (!version.IsInstalledInLauncher || !File.Exists(gamePath))
                throw new InvalidOperationException($"{version.Name} is not installed in its local version folder. Refusing to launch the Store-managed copy.");
            ...
            if (version.PackageType == PackageType.GDK)
            {
                ...
                {
                    FileName = gamePath,
                    WorkingDirectory = version.GameDirectory,
                    UseShellExecute = false
                });
                if (process == null)
                    throw new InvalidOperationException($"Could not start Minecraft {version.Name}.");

                // Process.Start only confirms creation, not that the game stayed open.
                // Never follow a Store/URI handoff to a different executable.
                if (!await LaunchProbe.StaysRunningAsync(process, TimeSpan.FromSeconds(5)))
                    throw new InvalidOperationException($"Minecraft {version.Name} exited during startup (exit code {process.ExitCode}). The requested local version did not stay running; bedli will not fall back to the Store or another version.");
                return;
            }

            // UWP requires package activation. Only activate a registration backed by
            // the requested local folder, never the Store-managed package family alone.
            Package package = PM.FindPackagesForUser(string.Empty, Constants.GetPackageFamily(version.Type))
                .FirstOrDefault(candidate => IsSamePackageVersion(candidate, version) &&
                    PathsEqual(GetPackageInstalledLocation(candidate), version.GameDirectory));
            if (package == null)
                throw new InvalidOperationException($"Minecraft {version.Name} is not registered from its local version folder. Refusing to activate a different installation.");

            if (!await TryLaunchPackageActivation(version, false))
                throw new InvalidOperationException($"Windows could not activate Minecraft {version.Name}.");

            DateTime deadline = DateTime.UtcNow.AddSeconds(10);
            while (DateTime.UtcNow < deadline)
            {
                foreach (Process process in Process.GetProcessesByName(Constants.MINECRAFT_PROCESS_NAME))
                {
                    using (process)
                    {
                        bool matches;
                        try { matches = PathsEqual(process.MainModule?.FileName, gamePath); }
            ...
            throw new InvalidOperationException($"Minecraft {version.Name} did not stay running from the requested folder. No fallback version was launched by bedli.");
        }

        private IEnumerable<string> GetRegisteredVCLibsRuntimeDirectories()
        {
            var directories = new List<string>();
            // Package locations are readable even when WindowsApps cannot be enumerated.
            ...
            })
            {
                try
                {
                    directories.AddRange(PM.FindPackagesForUser(string.Empty, family)
                        .Where(package => string.Equals(package.Id.Architecture.ToString(),
                            Constants.CurrentArchitecture, StringComparison.OrdinalIgnoreCase))
                        .Select(package => package.InstalledLocation.Path));
                }
                catch (Exception ex)
                {
                    Trace.WriteLine($"Could not locate registered VC runtime {family}: {ex.Message}");
                }
            }
        ...
        private async Task RunPowerShellPackageCommand(string command, string operationName)
        {
            ...
            if (process.ExitCode != 0)
            {
                ...
                if (Program.IsCommandLine && details.Contains("0x80073CFF", StringComparison.OrdinalIgnoreCase))
                    throw new InvalidOperationException("Windows requires Developer Mode to register this local Minecraft version (0x80073CFF). Open Windows Settings, search for Developer Mode, enable it, then retry the same bedli command. Your downloaded files are kept.");
        ...
        {
            if (Program.IsCommandLine) throw e;
