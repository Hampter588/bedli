...
try:
    run([str(entry),"--name=mclib","--onefile","--clean","--noconfirm",f"--distpath={dist}",f"--workpath={build}",f"--specpath={root}","--collect-submodules=mclib"])
finally:
    entry.unlink(missing_ok=True)
exe=dist/"mclib.exe"
if not exe.exists(): raise SystemExit("mclib.exe was not produced")
print(exe)
