...
public static class Catalog {
 ...
 public static async Task<List<BedrockVersion>> GetAsync(){
   using var s=await Http.GetStreamAsync(Source); using var doc=await JsonDocument.ParseAsync(s); var list=new List<BedrockVersion>(); Walk(doc.RootElement,null,list);
   return list.GroupBy(x=>(x.Version,x.Arch,x.Channel)).Select(g=>g.First()).OrderByDescending(x=>Parse(x.Version)).ToList();
 }
 static void Walk(JsonElement e,string? key,List<BedrockVersion> list){
   if(e.ValueKind==JsonValueKind.Object){foreach(var p in e.EnumerateObject()) Walk(p.Value,p.Name,list); return;}
   if(e.ValueKind==JsonValueKind.Array && key!=null){
     ...
     if(urls.Count>0){foreach(var g in urls.GroupBy(Arch))list.Add(new(Display(key),key,g.Key,"release",g.ToList())); return;}
     foreach(var x in e.EnumerateArray()) Walk(x,key,list);
   }
 }
